...
Streamlit Community Cloud는 기본적으로 `streamlit run app.py`로 UI 앱을 실행합니다. 따라서 이 저장소를 Streamlit Cloud에 배포하면 대시보드, Canvas, 시나리오 타임라인, `Live Latest Polling 테스트` 패널은 사용할 수 있습니다.
...
| Streamlit 안에서 live latest 값 확인 | Streamlit UI의 `Live Latest Polling 테스트` 패널 |
...
3. Streamlit에서 `시뮬레이션 실행`을 누르거나 `자동 실행`을 켭니다. 이때 현재 센서 snapshot이 `.runtime/current_snapshot.json`에 저장됩니다.

4. Postman 또는 백엔드에서 polling:
